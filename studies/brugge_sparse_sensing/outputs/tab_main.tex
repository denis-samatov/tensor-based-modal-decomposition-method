...
\caption{Pressure RMSE ($u_p$, the supplied export unit) in protocol P2, mean\,$\pm$\,standard deviation over the ten held-out control scenarios. Placement is QR (grid) or DG (wells) on the basis of the same row unless stated; $r=16$ in every fold. The prior row uses no measurements and is repeated across columns. Random: per-fold median over 20 placements.}
\label{tab:main}
